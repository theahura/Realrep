...
function loadMap() {
	...
	socket.emit('clientToServer', {
		...
		global_correlationTopFive = sortedKeys.slice(sortedKeys.length-5, sortedKeys.length);

		for(var i = 1; i <= 5; i++) {
			$('.bubble' + i).html(global_correlationTopFive[i - 1]);
		}

		$('.userbubble').html($('#SearchForCorrelation').val());
